...
ngx_rtmp_live_create_app_conf(ngx_conf_t *cf)
{
    ...
    lacf->relay_state_poll_len = NGX_CONF_UNSET;
...
ngx_rtmp_live_merge_app_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    // 转推是否开启
    ngx_conf_merge_value(conf->relay_state, prev->relay_state, 0);
    ngx_conf_merge_msec_value(conf->relay_state_poll_len, prev->relay_state_poll_len, 10000);
                         ...
                         unsigned active)
{
    ...
    if( ctx->newflag == 1 || !(lacf->idle_streams && lacf->push_cache && lacf->publish_delay_close)
...
ngx_rtmp_live_close_stream(ngx_rtmp_session_t *s, ngx_rtmp_close_stream_t *v)
{
    ...
    if (ctx->publishing) {
        ngx_rtmp_send_status(s, "NetStream.Unpublish.Success",
                "status", "Stop publishing");
        // 1. 立即清空stream。lacf->idle_streams:0 立即清除， 1:不立即清楚
        ...
        if ( codec_ctx->meta ) {
            ...
            if( lacf->push_cache ){
                if(!ctx->stream->meta && meta){
                    ...
                    ctx->stream->meta_version = meta_version;
                }
            }
        }
        //ctx->stream->coheader = ngx_rtmp_append_data_to_push_cache(cscf->chunk_size, ctx->stream, NULL, coheader);
    }
                 ...
                 ngx_chain_t *in)
{
    ...
    for (pctx = stream->ctx; pctx; pctx = pctx->next) {
        ...
        if (stream->meta && stream->meta_version != pctx->meta_version) {
            if (ngx_rtmp_send_message(ss, stream->meta, 0) == NGX_OK) {
                // 改变状态
                pctx->meta_version = stream->meta_version;
            }
        }

        if (!cs->active) {
            if( stream->aac_header && stream->avc_header){
                if ( h->type == NGX_RTMP_MSG_AUDIO ) {
                    if (apkt == NULL) {
...
ngx_rtmp_live_av_dump_cache_frame(ngx_rtmp_live_stream_t *stream)
{
    ...
    if( pc != NULL ){
        ...
        if ( stream->closed_count == 0 && stream->session != NULL ) {
            ...
            ngx_rtmp_live_av_to_play(stream, &pc->frame_header, pc->frame_buf);
            ...
            if ( pc->has_closed == 1 ){
                ...
                if(stream->closed_count == 0){
                    ...
                    if ( stream->avc_header )
                        ngx_rtmp_free_frame_buffer(stream, stream->avc_header);
                    stream->aac_header = NULL;
                    stream->avc_header = NULL;
                }
...
nxg_rtmp_live_av_dump_cache(ngx_event_t *ev)
{
    ...
    // 是否开启转推    
    if( stream->relay_ctrl && !stream->is_start_relay ){
                 ...
                 ngx_chain_t *in)
{
    ...
    if( !ev->timer_set && mandatory == 0 && pcp && pc){
...
nxg_rtmp_live_wait_relay(ngx_event_t *ev)
{
    ...
    file.name = stream->lacf->relay_state_file;
    file.log = NULL;
    ...
    //printf("LLLLL relay_ctrl:%ld\n", stream->relay_ctrl);
    /*
    buf[0] = '0';
    if (ngx_write_file(&file, buf, 1, 0) == NGX_ERROR) {
        printf("LLLLL ngx_write_file\n");
        return;
    } 
    */
    ...
    ngx_add_timer(ev, stream->lacf->relay_state_poll_len); 
}
static ngx_int_t
                 ...
                 ngx_chain_t *in)
{
    ...
    if ( !ev->timer_set ){
        printf("LLLLL init timer set  relay_state_poll_len:%ld\n", lacf->relay_state_poll_len);
...
ngx_rtmp_live_publish(ngx_rtmp_session_t *s, ngx_rtmp_publish_t *v)
{
    ...
    if( lacf->push_cache ){
        ...
        //ctx->stream->publish   = v;
        //ngx_rtmp_stream_relay_publish(ctx->stream, v);
    }
next:
